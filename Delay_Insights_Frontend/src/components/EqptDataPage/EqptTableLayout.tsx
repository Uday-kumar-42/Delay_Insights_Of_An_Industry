...
const EqptTableLayout = React.memo(({ data }: TableLayoutProps) => {
      ...
      <table className="w-full text-sm text-left rtl:text-right text-gray-600">
        <thead className="text-xs text-gray-700 uppercase bg-yellow-200">
          <tr>
            ...
            <th className="px-4 py-3">Effective Duration</th>
          </tr>
        </thead>
        <tbody>
          {pageData.map((row, idx) => (
            <tr
              key={idx}
              className="bg-white border-b border-yellow-200 hover:bg-yellow-50"
            >
              <th
                scope="row"
...
export default EqptTableLayout;
